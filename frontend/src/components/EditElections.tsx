...
import { useUpdateElection, type Election } from '../hooks/ElectionHooks'
import { getImageUrl } from '../utils/imageUtils'

interface EditElectionsProps {
  open: boolean
  onClose: () => void
  onSuccess?: () => void
...
export default function EditElections({ open, onClose, onSuccess, election }: EditElectionsProps) {
  const [imagePreview, setImagePreview] = React.useState<string | null>(null)
  const fileInputRef = React.useRef<HTMLInputElement>(null)
  
  const { mutate: updateElection, isPending, error } = useUpdateElection()
  ...
  // Populate form when election changes
  React.useEffect(() => {
    if (election && open) {
      const startDateTime = parseDateTime(election.start_date)
      const endDateTime = parseDateTime(election.end_date)
      
      setValue('election_title', election.election_title)
      setValue('description', election.description)
      setValue('election_type', election.election_type)
      setValue('startDate', startDateTime.date)
      setValue('startTime', startDateTime.time)
      setValue('endDate', endDateTime.date)
      setValue('endTime', endDateTime.time)
      setValue('img', null)
      
      // Set image preview from existing image
      if (election.img_url) {
        setImagePreview(getImageUrl(election.img_url))
      } else {
        setImagePreview(null)
      }
    }
  }, [election, open, setValue])
                ...
                {imagePreview ? (
                  ...
                  <>
                    <input
                      ...
                      style={{ display: 'none' }}
                      id="edit-election-image-upload"
                    />
                    <label htmlFor="edit-election-image-upload">
                      <Button
                        variant="outlined"
                        component="span"
            ...
            <Controller
              ...
              render={({ field }) => (
                <FormControl fullWidth error={!!errors.election_type}>
                  <InputLabel id="edit-election-type-label">Election Type</InputLabel>
                  <Select
                    {...field}
                    labelId="edit-election-type-label"
                    label="Election Type"
                    value={field.value}
